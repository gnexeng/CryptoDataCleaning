...
Recall that one theme of the paper is to show that the NIST test suite cannot detect certain non-random sequences that arise from combinatorial constructions. We conducted three sets of experiments, representing tests along three different orthogonal axes, the $h$-vector length, the number of coning operations, and the contents of the $h$-vector. Our first experiment follows these steps:
...
A non-random pattern should be consistently detected by the NIST test suite and shown with every data set having a $p$-value of close to 1.0 or close to 0.0, which will appear as a ``clustering'' of points towards the top or bottom of the graph, also resembling a flatline (See Figure~\ref{fig:random} and ~\ref{fig:nonrandom}). These sparklines represent the trend of all NIST tests for different variations of the input $h$-vectors. The time required to run these tests on a modern consumer CPU (Intel Core i5-4590) was originally 2.3 seconds as per our previous publication~\cite{ALDH}. However, performance of the code improved due to optimizations and significant algorithmic improvements. We have also updated the results from our previous publication due to these code improvements, and we added samples of the results to the Appendix~\ref{second-appendix}. 

For the first experiment, we fix an $h$-vector of length 3751 consisting of all ones.  So this is the $h$-vector of $\Delta^{3751}$.  We proceed to steps 2 and 3 and apply the cone to the dual simplicial complex then compute its $f$-vector.  Vary the number of coning operations from 0 to 99 times on the dual complex $\mathcal{K}$ for $0 \leq k \leq 99$.  See Figure~\ref{fig:conings}) in Appendix~\ref{first-appendix} and the sample scatter plots on Figures~\ref{fig:approxentropy21},~\ref{fig:linearcomplexity21} and~\ref{fig:rank21}, which show the $p$-value distributions and also illustrate the fact that the NIST test suite is unable to detect non-randomness.   %    '-p 3751 -f -c 0 -b' through '-p 3751 -f -c 99 -b'
...
\clearpage
\indent Motivated by the failure of the NIST test suite to detect non-random phenomena we conducted a third experiment. Here, the $h$-vector was fixed at a length of 3750 and we varied the non-end elements in the pattern from 1 to 100. The contents of the $h$-vector can be all-1’s, or it can be any other integer as long as the two end elements are 1. Varying the non-end elements from 1 to 100 literally means creating the following $h$-vectors: 
