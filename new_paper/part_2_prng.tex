...
\sloppy
The algorithms described in the previous section allow us to perform different operations on simplicial complex vectors. These vectors will increment in size as a result of the coning operation, but we can also start with a large vector to operate. The resulting vectors are converted to a stream of bits, which we then use as input to the National Institute of Standards and Technology (NIST) statistical test suite. You can download the suite at the following url:  \url{https://csrc.nist.gov/Projects/Random-Bit-Generation/Documentation-and-Software} 

The suite requires an bit stream input from a random number generator that contains at least 10 megabits of input data.  For each each bit stream, the NIST test suite will output a report that shows in detail the p-values associated with each of the tests that the suite contains. 

%Rerunning experiments using version 2.0 of the code.
Recall, one theme of the paper is to show that the NIST test suite cannot detect certain non-random sequences that arise form combinatorial constructions.  To this end we conducted three experiments following these steps:
...
\item Step 2:  Compute the dual f-vector corresponding to the h-vector from step one.  
\item Step 3:  Compute the kth cone on the f-vector from the previous step.  i.e., $C^k(K)$ where $K$ is the simplicial complex dual to $\Delta^n$ and $ 0 \leq k \leq q$ for some $q$. (first experiment only)
...
A non-random pattern should be consistently detected by the NIST test suite and shown with every data set having a p-value of close to 1.0 or close to 0.0, which will appear as a ``clustering'' of points towards the top or bottom of the graph (See Figure~\ref{fig:random} and ~\ref{fig:nonrandom}). The sparklines represent the trend of all NIST tests, which if close to 0.0 or 1.0 would look like a flatline. The time required to do this using a naive implementation on a modern consumer CPU (Intel Core i5-4590) is 2.3 seconds, which is consistent with our previous tests performed in~\cite{ALDH}. This is therefore a gross upper bound. %\textcolor{red}{is this an obvious fact?  If so, ok to leave.  If not, then some explanation is needed here}


For the first experiment, we fix an h-vector of length 3751 consisting of all ones.  So this is the h-vector of $\Delta^{3751}$.  We proceed to steps 2 and 3 and apply the cone to the dual f-vector.  Let $0 \leq k \leq 99$.  Namely,  vary the number of coning operations from 0 to 99 times on the dual complex $K$ (see Figure~\ref{fig:conings}) Also see the sample scatter plots on Figures~\ref{fig:approxentropy21},~\ref{fig:linearcomplexity21} and~\ref{fig:rank21}, which show the p-value distributions and also illustrate the fact that the NIST test suite is unable to detect non-randomness.   %    '-p 3751 -f -c 0 -b' through '-p 3751 -f -c 99 -b'
...
\begin{figure}[h!]
\centering
\caption{ApproximateEntropy NIST test p-values after applying between 0 - 99 coning operations on a vector of length 3751 with a pattern of all 1's}
...
\begin{figure}[h!]
\centering
\caption{LinearComplexity NIST test p-values after applying between 0 - 99 coning operations on a vector of length 3751 with a pattern of all 1's}
...
\clearpage
\indent For the second experiment,  we decided to experiment with the NIST suite with simply converting the h-vector to its dual and without any coning operation. This should serve as a benchmark since there is no random process involved, thus the NIST suite should effectively indicate that the bit stream is non-random.  To this end we now vary the h-vector length from 3750 to 3849. So we have a collection of h-vectors all of whose components are ones corresponding to a set of standard simplicies:  $\{\Delta^i \;| \; 3750 \leq i \leq 3849\}$.  Here, the cone operation is not applied to the dual. However, the NIST test suite is unable to detect the non-randomness as it is seen in the sparklines in Figure~\ref{fig:lengths}.  We have also included scatter plots for the Approximate Entropy test (Figure~\ref{fig:approxentropy22}, the Linear Complexity test (Figure~\ref{fig:linearcomplexity22} and the Rank test (Figure~\ref{fig:rank22}.  %  '-p 3750 -f -b' through '-p 3849 -f -b'
...
\begin{figure}[h!]
\centering
\caption{ApproximateEntropy NIST test p-values on a vectors of length between 3750 and 3849 with a pattern of all 1's, without applying coning operations}
...
\begin{figure}[h!]
\centering
\caption{LinearComplexity NIST test p-values on a vectors of length between 3750 and 3849 with a pattern of all 1's, without applying coning operations}
...
\clearpage
\indent For our third and final experiment, again intrigued by the failure of the NIST suite to detect non-randomness in the previous run, we fix the h-vector length at 3750 and vary non-end \textcolor{red}{Explain what is meant by non-end elements and provide an exmaple.  This is not clear to me} elements in the pattern from 1 to 100.  The cone is not applied to the dual. See Figures~\ref{fig:patterns},~\ref{fig:approxentropy23},~\ref{fig:linearcomplexity23} and~\ref{fig:rank23}  %   '-p 3750 -i 1 -f -b' through '-p 3750 -i 100 -f -b'
