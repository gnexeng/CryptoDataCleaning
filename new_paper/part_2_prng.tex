...
For the first experiment we fix the h-vector length at 3751, with a pattern of all 1's, and vary the number of coning operations from 0 to 99 times.~\ref{fig:conings}  %    '-p 3751 -f -c 0 -b' through '-p 3751 -f -c 99 -b' 
...
\newcommand{\captionabove}[2][]{%
 ...
 \centering
 \captionabove{Trendlines for exeriments with varying conings from 0-99 times (left), varying h-vector lengths from 3750-3849 (mid) and varying patterns for h-vector length of 3750 and non-end elements from 1-100 (right).}%
 \label{fig:images}
 \subfloat[left]{%
