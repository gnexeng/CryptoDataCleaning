...
Since the appearance of ~\cite{ALDH} the code has been updated.  Version 2.0 of the code fixes several bugs, including one where 0's were not being properly appended to the bit-stream. As a result, fewer false positives appear in the output from the NIST test suite. The updated code was used to run all the tests in this paper and to re-run to tests/xperiments from and the updated results from ~\cite{ALDH}.  The interested reader can refer to the following GitHub repository:
\begin{center}
