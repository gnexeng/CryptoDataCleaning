...
Recall, one theme of the paper is to show that the NIST test suite cannot detect certain non-random sequences that arise from combinatorial constructions.  We conducted three sets of experiments, representing tests along three different orthogonal axes, the h-vector length, the number of coning operations, and the contents of the h-vector. Our experiments follow these steps:
...
\clearpage
\indent Motivated by the failure of the NIST test suite to detect non-random phenomena a third experiment was conducted.  Here, the $h$-vector was fixed at a length of 3750 and we varied the non-end elements in the pattern from 1 to 100.  

% To be edited. 
%The contents of the h-vector can be all-1’s, or it can be any other integer as long as the two end elements are 1. From the command-line flag “-i” in those comments, this must have been version 2.1.0 or earlier. “-i 4” in that version meant “make all the interior elements have the value 4”. 

The cone is not applied to the dual. As with the previous two experiments, we included the sparklines (see Figure~\ref{fig:patterns} in Appendix~\ref{first-appendix}) and three scatter plots for the Approximate Entropy test (see Figure~\ref{fig:approxentropy23}), the Linear Complexity test (see Figure~\ref{fig:linearcomplexity23}) and the Rank test (see Figure~\ref{fig:rank23}).  %   '-p 3750 -i 1 -f -b' through '-p 3750 -i 100 -f -b'
